...
fn float_fn(inputs: impl Into<TypeRow>, outputs: impl Into<TypeRow>) -> FunctionType {
    FunctionType::new(inputs, outputs).with_extension_delta(float_types::EXTENSION_ID)
}
...
fn test_big() {
    ...
    */
    ...
    let mut build = DFGBuilder::new(float_fn(type_row![], vec![sum_type.clone().into()])).unwrap();
...
fn test_folding_pass_issue_996() {
    ...
    // x7 := or(x4, x6) // true
    // output x7
    let mut build = DFGBuilder::new(float_fn(type_row![], vec![BOOL_T])).unwrap();
...
fn test_const_fold_to_nonfinite() {
    ...
    let mut build = DFGBuilder::new(float_fn(type_row![], vec![FLOAT64_TYPE])).unwrap();
...
fn int_fn(inputs: impl Into<TypeRow>, outputs: impl Into<TypeRow>) -> FunctionType {
    FunctionType::new(inputs, outputs).with_extension_delta(int_types::EXTENSION_ID)
}
...
fn test_fold_iwiden_u() {
    ...
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![INT_TYPES[5].clone()])).unwrap();
...
fn test_fold_inarrow<I: Copy, C: Into<Value>, E: std::fmt::Debug>(
    ...
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![sum_type.clone().into()])).unwrap();
...
fn test_fold_itobool() {
    ...
    // x1 := itobool(x0);
    // output x1 == true;
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![BOOL_T])).unwrap();
...
fn test_fold_idivmod_s(#[case] a: i64, #[case] b: u64, #[case] c: i64) {
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![INT_TYPES[6].clone()])).unwrap();
...
fn test_fold_imod_u() {
    ...
    // x2 := imod_u(x0, x1);
    // output x2 == int_u<3>(2);
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![INT_TYPES[3].clone()])).unwrap();
    let x0 = build.add_load_const(Value::extension(ConstInt::new_u(5, 20).unwrap()));
    let x1 = build.add_load_const(Value::extension(ConstInt::new_u(3, 3).unwrap()));
    let x2 = build
...
fn test_fold_itostring_u() {
    ...
    // x1 := itostring_u(x0);
    // output x2 := "17";
    let mut build = DFGBuilder::new(int_fn(type_row![], vec![STRING_TYPE])).unwrap();
