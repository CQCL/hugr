...
pub(super) fn run_datalog<V: AbstractValue, C: DFContext<V>>(
    ...
    let all_results = ascent::ascent_run! {
        ...
        node(n) <-- for n in ctx.hugr().nodes();

        in_wire(n, p) <-- node(n), for (p,_) in ctx.hugr().in_value_types(*n); // Note, gets connected inports only
        out_wire(n, p) <-- node(n), for (p,_) in ctx.hugr().out_value_types(*n); // (and likewise)

        parent_of_node(parent, child) <--
            node(child), if let Some(parent) = ctx.hugr().get_parent(*child);

        input_child(parent, input) <-- node(parent), if let Some([input, _output]) = ctx.hugr().get_io(*parent);
        output_child(parent, output) <-- node(parent), if let Some([_input, output]) = ctx.hugr().get_io(*parent);

        // Initialize all wires to bottom
        out_wire_value(n, p, PV::bottom()) <-- out_wire(n, p);

        in_wire_value(n, ip, v) <-- in_wire(n, ip),
            if let Some((m, op)) = ctx.hugr().single_linked_output(*n, *ip),
        ...
        in_wire_value(n, p, v) <-- for (n, p, v) in in_wire_value_proto.iter(),
          node(n),
          if let Some(sig) = ctx.hugr().signature(*n),
          if sig.input_ports().contains(p);

        // Assemble in_value_row from in_value's
        node_in_value_row(n, ValueRow::new(sig.input_count())) <-- node(n), if let Some(sig) = ctx.hugr().signature(*n);
        node_in_value_row(n, ValueRow::single_known(ctx.hugr().signature(*n).unwrap().input_count(), p.index(), v.clone())) <-- in_wire_value(n, p, v);

        out_wire_value(n, p, v) <--
           node(n),
           let op_t = ctx.hugr().get_optype(*n),
        ...
        dfg_node(n) <-- node(n), if ctx.hugr().get_optype(*n).is_dfg();
        ...
        out_wire_value(i, OutgoingPort::from(p.index()), v) <-- node(tl),
            if ctx.hugr().get_optype(*tl).is_tail_loop(),
            input_child(tl, i),
        ...
        out_wire_value(in_n, OutgoingPort::from(out_p), v) <-- node(tl),
            if let Some(tailloop) = ctx.hugr().get_optype(*tl).as_tail_loop(),
            input_child(tl, in_n),
        ...
        conditional_node(n)<-- node(n), if ctx.hugr().get_optype(*n).is_conditional();
        case_node(cond, i, case) <-- conditional_node(cond),
          for (i, case) in ctx.hugr().children(*cond).enumerate(),
          if ctx.hugr().get_optype(case).is_case();
        ...
        out_wire_value(i_node, OutgoingPort::from(out_p), v) <--
          ...
          let conditional = ctx.hugr().get_optype(*cond).as_conditional().unwrap(),
        ...
        cfg_node(n) <-- node(n), if ctx.hugr().get_optype(*n).is_cfg();

        // In `CFG` <Node>, basic block <Node> is reachable given our knowledge of predicates
        relation bb_reachable(Node, Node);
        bb_reachable(cfg, entry) <-- cfg_node(cfg), if let Some(entry) = ctx.hugr().children(*cfg).next();
        ...
        _cfg_succ_dest(cfg, exit, cfg) <-- cfg_node(cfg), if let Some(exit) = ctx.hugr().children(*cfg).nth(1);
        _cfg_succ_dest(cfg, blk, inp) <-- cfg_node(cfg),
            for blk in ctx.hugr().children(*cfg),
            if ctx.hugr().get_optype(blk).is_dataflow_block(),
            input_child(blk, inp);

        // Outputs of each reachable block propagated to successor block or CFG itself
        out_wire_value(dest, OutgoingPort::from(out_p), v) <--
            bb_reachable(cfg, pred),
            if let Some(df_block) = ctx.hugr().get_optype(*pred).as_dataflow_block(),
            for (succ_n, succ) in ctx.hugr().output_neighbours(*pred).enumerate(),
        ...
        func_call(call, func_defn) <--
            node(call),
            if ctx.hugr().get_optype(*call).is_call(),
            if let Some(func_defn) = ctx.hugr().static_source(*call);
