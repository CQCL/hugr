...
pub(in crate::hugr::patch) mod test {
    ...
    fn test_replace_with_function(dfg_hugr_copy_bools: (Hugr, Vec<Node>)) {
        let replacement = {
            ...
            let fb = mb
                .define_function("my_id", Signature::new_endo(bool_t()))
                .unwrap();
            let [inp] = fb.input_wires_arr();
            let f_id = fb.finish_with_outputs([inp]).unwrap();
            let [b] = dfb
                .call(f_id.handle(), &[], dfb.input_wires())
                .unwrap()
                .outputs_arr();
        ...
        subject
            .apply_patch(SimpleReplacement {
                subgraph,
                replacement,
            })
            .unwrap();
        subject.validate().unwrap();
        let func_body: [Node; 3] = subject
            .children(subject.entrypoint())
            .collect_array()
            .unwrap();
        assert_eq!(
            func_body.map(|n| subject.get_optype(n).tag()),
            [OpTag::Input, OpTag::Output, OpTag::FnCall]
        );
        ...
        assert_eq!(
            subject
                .get_optype(new_func)
                .as_func_defn()
                .map(FuncDefn::func_name),
            Some(&"my_id".to_string())
        );
        assert_eq!(
            subject.static_targets(new_func).unwrap().collect_vec(),
            [(func_body[2], 1.into())]
        );
    }
