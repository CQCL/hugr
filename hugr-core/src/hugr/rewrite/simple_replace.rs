...
pub(in crate::hugr::rewrite) mod test {
    ...
    use crate::builder::{
        ft1, ft2, BuildError, Container, DFGBuilder, Dataflow, DataflowHugr, DataflowSubContainer,
    ...
    fn make_dfg_hugr() -> Result<Hugr, BuildError> {
        let mut dfg_builder = DFGBuilder::new(ft1(type_row![QB, QB]))?;
    ...
    pub(in crate::hugr::rewrite) fn dfg_hugr_copy_bools() -> (Hugr, Vec<Node>) {
        fn build() -> Result<(Hugr, Vec<Node>), BuildError> {
            let mut dfg_builder =
                DFGBuilder::new(ft2(type_row![BOOL_T], type_row![BOOL_T, BOOL_T]))?;
    ...
    fn test_replace_cx_cross() {
        ...
        let mut builder = DFGBuilder::new(ft1(q_row)).unwrap();
    ...
    fn test_replace_after_copy() {
        ...
        let mut builder = DFGBuilder::new(ft1(one_bit.clone())).unwrap();
        ...
        let mut builder = DFGBuilder::new(ft2(two_bit, one_bit)).unwrap();
