...
impl HugrMut for Hugr {
    ...
    fn insert_forest(
        ...
        let node_map = match roots.size_hint().1 {
            Some(0) => return Ok(HashMap::new()), // nothing to do
            Some(1) => {
                // Skip DoubleCopy check and avoid allocating singleton HashMap
                let Some((root, parent)) = roots.next() else {
                    return Ok(HashMap::new());
                };
                insert_hugr_internal(
                    self,
                    &other,
                    other.descendants(root),
                    std::iter::once((root, parent)),
                )
            }
            _ => {
                let map: HashMap<Node, Self::Node> = roots.collect();
                for &r in map.keys() {
                    let mut n = r;
                ...
                insert_hugr_internal(
                    self,
                    &other,
                    map.keys().flat_map(|n| other.descendants(*n)),
                    map.iter().map(|(r, p)| (*r, *p)),
                )
            }
        }
