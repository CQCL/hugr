...
pub struct Walker<'a> {
    ...
    /// selected.
    ///
    /// Note that we could store this as a BTreeSet<CommitId>, but it is very
...
impl PersistentHugr {
    ...
    fn get_single_outgoing_port(
        ...
        assert!(!self.contains_node(node), "node must be in current hugr");

        use itertools::Either::{Left, Right};
        let mut node = node;
        let mut port = match port.into().as_directed() {
            Left(incoming) => incoming,
            Right(outgoing) => {
                // nothing to do
