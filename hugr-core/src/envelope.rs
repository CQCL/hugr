...
#[deprecated(since = "0.24.1", note = "Use PackageDesc::generator instead")]
pub fn get_generator<H: HugrView>(modules: &[H]) -> Option<String> {
    let mut desc: PackageDesc = PackageDesc::default();
    desc.set_n_modules(modules.len());

    for (module, d) in modules.iter().zip(desc.modules.iter_mut()) {
        let d = d.get_or_insert_default();
        d.load_generator(module);
    }
...
fn gen_str(generator: Option<&str>) -> String {
    match generator {
        Some(g) => format!("\ngenerated by {g}"),
        None => String::new(),
    }
...
#[error("{inner}{}", gen_str(self.generator().map(|s| s.as_str())))]
