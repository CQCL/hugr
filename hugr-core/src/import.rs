...
impl<'a> Context<'a> {
    ...
    ///
    /// In `hugr-core`, the first node in the region is a normal basic block
    /// operation but by virtue of its position is also marked the entry block.
    /// The second node in the region is an [`ExitBlock`].
