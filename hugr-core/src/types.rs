...
use crate::types::type_param::{TermEnum, check_term_type};
    ...
    fn substitute(&self, t: &Substitution) -> Vec<Self> {
        match &self.0 {
            ...
            TypeEnum::Variable(idx, bound) => {
                ...
                let Some(ty) = term.as_runtime() else {
    ...
    fn apply_rowvar(&self, idx: usize, bound: TypeBound) -> Vec<TypeRV> {
        ...
        match arg.get() {
            TermEnum::List(elems) => elems
                ...
                .map(|ta| {
                    match ta.get() {
                        TermEnum::Runtime(ty) => return ty.clone().into(),
                        TermEnum::Variable(v) => {
                ...
                .collect(),
            TermEnum::Runtime(ty) if matches!(ty.0, TypeEnum::RowVar(_)) => {
