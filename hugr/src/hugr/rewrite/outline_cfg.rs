...
mod test {
    ...
    fn test_outline_cfg_errors() {
        ...
        let [left, right] = h.output_neighbours(entry).collect_vec().try_into().unwrap();
        let r = h.apply_rewrite(OutlineCfg::new([entry, left, right]));
        let Err(OutlineCfgError::MultipleExitNodes(a, b)) = r else {
            panic!("Did not get expected error {:?}", r)
        };
        assert_eq!(HashSet::from([a, b]), HashSet::from_iter([left, right]));
        assert_eq!(h, backup);

        let r = h.apply_rewrite(OutlineCfg::new([left, right, merge]));
        let Err(OutlineCfgError::MultipleEntryNodes(a, b)) = r else {
            panic!("Result {:?} was not expected error", r)
        };
