...
pub mod test {
    ...
    fn gen_optype(g: &MultiPortGraph, node: NodeIndex) -> OpType {
        ...
        match (inputs == 0, outputs == 0) {
            (false, false) => DFG {
                signature: AbstractSignature::new(vec![NAT; inputs - 1], vec![NAT; outputs - 1]),
            }
            .into(),
    ...
    fn weighted_hugr_ser() {
        let hugr = {
            ...
            let mut f_build = module_builder
                .define_function("main", AbstractSignature::new(t_row.clone(), t_row).pure())
                .unwrap();
    ...
    fn dfg_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut dfg = DFGBuilder::new(AbstractSignature::new(tp.clone(), tp))?;
