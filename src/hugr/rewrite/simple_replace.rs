...
impl Rewrite<SimpleReplacementError> for SimpleReplacement {
    ...
    fn apply(self, h: &mut Hugr) -> Result<(), SimpleReplacementError> {
        ...
        let self_input_node_index = h.hierarchy.first(self.parent.index).unwrap();
        let replacement_output_node = *replacement_nodes.last().unwrap();
        for &node in replacement_inner_nodes {
            // Add the nodes.
            let op: &OpType = self.replacement.get_optype(node);
            let sig = op.signature();
            let new_node_index = h.graph.add_node(sig.input.len(), sig.output.len());
            h.op_types[new_node_index] = op.clone();
        ...
        for &node in replacement_inner_nodes {
            ...
            for node_successor in self.replacement.output_neighbours(node) {
