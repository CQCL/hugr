...
use hugr_core::{
    ...
    core::HugrNode,
    hugr::{HugrError, views::HierarchyView},
    ops::{CFG, DataflowBlock, ExitBlock, Input, NamedOp, OpType, Output},
    types::Type,
};
...
///
/// We provide [`PartialEq`], [Eq], [`PartialOrd`], [Ord], [Hash], so that this
/// type can be used in containers. Note that these implementations use only the
