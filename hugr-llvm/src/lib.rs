...
//!
//! References:
//! * The full specification for LLVM IR as on the main branch:
//!   <https://llvm.org/docs/LangRef.html>.
//! * The full specification for LLVM IR version for version 14.0:
//!   <https://releases.llvm.org/14.0.0/docs/LangRef.html>
//! * The documentation for the [inkwell] crate
...
//!   they do not outlive their owning `Context`. A `Context` is not thread
//!   safe. A Context is used to construct modules, types, builders, and basic
//!   blocks.
//!
...
//!   attributes. It may contain basic blocks, in which case compiling the
//!   owning module will produce object code for the function. If it does not
//!   contain basic blocks then it represents an external symbol that can be
//!   called, and object code for that symbol must be linked with the object
//!   code from this module.
//!
//! * [Instruction](inkwell::values::InstructionOpcode): A basic block is an
//!   ordered list of instructions. Examples: `load`, `store`, `iadd`, `ret`.
//!   They are all defined in the `LangRef`.
//!
