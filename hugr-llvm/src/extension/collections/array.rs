...
pub fn build_array_fat_pointer<'c, H: HugrView<Node = Node>>(
    ...
    let array_ty = array_fat_pointer_ty(&ctx.typing_session(), elem_ty);
...
pub fn build_array_alloc<'c, H: HugrView<Node = Node>>(
    ...
    let array_v = build_array_fat_pointer(ctx, elem_ty, elem_ptr, offset)?;
    Ok((elem_ptr, array_v))
}
...
fn emit_pop_op<'c, H: HugrView<Node = Node>>(
    ...
    let ret_ty = ts.llvm_sum_type(option_type(vec![
        elem_ty.clone(),
        array_type(size.saturating_add_signed(-1), elem_ty.clone()),
    ]))?;
    if size == 0 {
    ...
    let (elem_ptr, new_array_offset) = {
        if pop_left {
            ...
            let elem_ptr = unsafe {
                builder.build_in_bounds_gep(ctx.llvm_type(&elem_ty)?, array_ptr, &[idx], "")
            }?;
            (elem_ptr, array_offset)
        }
    };
    let elem_llvm_ty = ctx.llvm_type(&elem_ty)?;
    let elem_v = builder.build_load(elem_llvm_ty, elem_ptr, "")?;
    let new_array_v = build_array_fat_pointer(ctx, elem_llvm_ty, array_ptr, new_array_offset)?;
...
pub fn emit_scan_op<'c, H: HugrView<Node = Node>>(
    ...
    build_loop(ctx, array_len, |ctx, idx| {
        ...
        // Build function type for indirect call (returning struct of tgt + accs if >1 outputs)
